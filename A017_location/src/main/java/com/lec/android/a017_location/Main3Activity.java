...
public class Main3Activity extends AppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        btnGeoCoder1 = findViewById(R.id.btnGeoCoder1); // 경도위도
        btnGeoCoder2 = findViewById(R.id.btnGeoCoder2); // 주소
        btnMap1 = findViewById(R.id.btnMap1); // 경도위도 지도
        btnMap2 = findViewById(R.id.btnMap2); // 주소지도
            ...
            public void onClick(View v) {
                ...
                try {
                    ...
                    list =  geocoder.getFromLocation(
                            lat, // 위도
                            lng, // 경도
                            10);// 얻어올 결과 값의 최대 개수(특정정보가 여러개일수 있음)
                } catch (IOException e) {
                    e.printStackTrace();
                    ...
                    if(list.size() == 0){
                        ...
                        for(Address addr : list){
                            ...
                            result.append(addr.getCountryName() + ", "+ addr.getFeatureName()  + ", "
                                    + addr.getLocality() + ", "  + addr.getLatitude() + ", " + addr.getLongitude() + "\n");
                        }
        ...
        btnMap1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // 위도 경도 입력후 지도 버튼 클릭 -> 지도 앱 화면으로 묵시적 인텐트 날라기
                double lat = Double.parseDouble(etLatitude.getText().toString());
                double lng = Double.parseDouble(etLongitude.getText().toString());
            ...
            public void onClick(View v) {
                ...
                if(list !=null){
                    if(list.size() == 0){
                        ...
                        // 해당되는 주소의 위치 좌표로 묵시적 인텐트 날리기(자도앱)
