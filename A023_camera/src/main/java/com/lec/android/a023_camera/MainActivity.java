...
import java.io.File;
/*  오늘날 스마트폰의 카메라 용도
   단순 사진 촬영을 넘어, 바코드, 문자 인식, AR (증강현실)등 광범위하게 사용
...
public class MainActivity extends AppCompatActivity {
    ...
    public void takePicture(){
        ...
        Uri fileUri = FileProvider.getUriForFile(this,
                ...
                file);
        // 사진 촬영 앱 가동(묵시적 인텐트)
        Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE); // 사진촬영앱을 띄울거임
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        if(requestCode == 101 && resultCode == RESULT_OK){
            // 저장된 사진 이미지 파일을 BitMap 객체로 만들고, ImageView 에 띄우기
            BitmapFactory.Options options = new BitmapFactory.Options();
            options.inSampleSize = 8;// 좀작은 사이즈로 resample을 위한 samplesize
            Bitmap bitmap = BitmapFactory.decodeFile(file.getAbsolutePath(), options); //저장경로에서 저장된사진 가져오기
