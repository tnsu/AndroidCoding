...
// Adapter 객체정의
// 데이터(Phonebook)를 받아서, 각 item별로 View를 생성
public class PhonebookAdapter extends RecyclerView.Adapter<PhonebookAdapter.ViewHolder> { //PhonebookAdapter.ViewHolder 안에 홀더
    ...
    // Adapter 생성자
    public PhonebookAdapter() {this.adapter = this;} // 자기자신이 갖고있는 리스트에 있는 값들을 받기 위해서
...
//  이로부터 Context 객체를 뽑아내어 Layout inflation 을 해야 한다.
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        // xml layout 를 Inflater 를 통해  Viewlayout변환
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        Phonebook item = items.get(position); // List<> 의 get() 사용
        holder.setItem(item);
    }
    ...
    // nested class(static class)로 ViewHolder 클래스 정의
        ...
        public ViewHolder(@NonNull View itemView) {  // item 레이아웃의 View 객체가 전달됨
            ...
            btnDelete.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    //데이터 변경(수정,삭제, 추가) 내역이 adapter 에 반영되어야 정상적으로 동작한다.
                    adapter.notifyDataSetChanged(); // 완전히 사라지게 함
                }
            });

            // 클릭리스너 장착 (상세페이지로 이동)
            //각 item 을 ‘클릭’ 했을때 수행하는 이벤트 리스너 만들기
            //클릭이벤트는 RecyclerView 에 발생하는 것이 아니라,각 item 에 발생함
            //따라아서 ViewHolder 에서 처리해주어야 한다.
            itemView.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                 ...
                 //   Toast.makeText(v.getContext(), "position:" + position, Toast.LENGTH_SHORT).show();
                    ...
                    // 아이템을 클릭하면 해당 세부 정보 엑티비티로 넘겨주기
                    Intent intent = new Intent(v.getContext(), PhonebookDetail.class); // getapp 이 안되서 v.getContext()
                    intent.putExtra("pb", adapter.getItem(position));

                    v.getContext().startActivity(intent); //액티비티가 아닌 곳에서 사용할때 v.getContext(). 넣어주고 사용하면 됨
                }
            });
