...
//      Result(결과 : onPostExecute 에서 사용할 변수의 타입
    ...
    class BackgroundTask extends AsyncTask<Integer, Integer, Integer>{
        ...
        // 백 그라운드 작업 : 반드시 구현!! // 처음 실행 할때 바로 나오는 것
        @Override
        protected Integer doInBackground(Integer... integers) { // 가변 매개변수, Integer 는 Integer[]
           // 여기가 본론
            for (backValue1 = 0; backValue1 < integers[0]; backValue1++){
                if(backValue1 % 10 == 0){
                    publishProgress(backValue1); // progress 상태를 update 뽑아냄 --> ProgressUpdate 호출되고 매개변수 값 전달됨
                }
                try {
                    Thread.sleep(100);
            ...
            return backValue1; // onPostExcute에 넘어가는 값

            // ※ doInBackground() 에서 시간이 많이 걸리는 '통신' 작업이나 복잡한 연산 작업등을 (비동기로)수행케 해야 한다.
        }

       // 백그라운드 작업도중 (여러번) 호출 가능, 진행 상황 업데이트, 중간중간 UI 업데이트시 사용
        @Override
        protected void onProgressUpdate(Integer... values) {
           Log.d("myapp","Progress : " + values[0] + "%"); // values : PublishProgress(i) 가 보낸 값
            super.onProgressUpdate(values);

            tvBackValue1.setText("onProgressUpdate : " + values[0]); // ★ Handler 없이도 메인 UI 접근가능 !!!!하이라이트!!
        }
        ...
        protected void onPostExecute(Integer integer) { // doInBackground 에서 return 한 값을 매개변수로 받는다.
            ...
            tvBackValue1.setText("onPostExecute:" + integer); // ★ Handler 없이도 메인 UI 접근가능
        }
    }// end Acy
