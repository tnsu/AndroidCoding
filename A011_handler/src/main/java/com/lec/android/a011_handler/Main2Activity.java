...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //방법3
        BackThread3 thread3 = new BackThread3(handler3); //handler3 메인스레드의 Handler 객체를 외부 클래스에 넘겨줌
        thread3.setDaemon(true);
        thread3.start();

        //방법4
        BackThread4 thread4 = new BackThread4(handler4); //handler3 메인스레드의 Handler 객체를 외부 클래스에 넘겨줌
    ...
    class BackThread1 extends Thread{
        ...
        public void run() {
            while(true){
              // 방법 1) 메인에서 생성된 Hendler 객체의 sendEmptyMessage를 통해 Message 전달
                backValue1++;
                handler1.sendEmptyMessage(1);


              // 방법2) 메인에서 생성된 Handler 샛페의 postXXX() 를 통한 Runnable 객체 전달
                backValue2 +=2;
                handler2.post(new Runnable() {
                    @Override
                    public void run() { // Runnable 의 run() 에서 메인 UI접근
    ...
    Handler handler1 = new Handler(){
        ...
        public void handleMessage(@NonNull Message msg) {
            if(msg.what == 1){ // Message id가 1 이면
                tvBackValue1.setText("BackValue1 : " + backValue1); // 메인스레드 UI변경
            }
        }
    }; // 익명클래스
