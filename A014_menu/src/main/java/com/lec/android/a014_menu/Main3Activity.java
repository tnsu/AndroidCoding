package com.lec.android.a014_menu;
...
//        abar.hide(); // ActionBar 감추기
//        abar.show(); // ActionBar 보이기기
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        //#2 ActionBar 에 EditText 로 입력 가능
        View v = menu.findItem(R.id.menu_search).getActionView(); // 메뉴안에 검색 item 을 가져옴

        if(v != null){  // v null아닐때
           EditText etText = v.findViewById(R.id.etText); // v(메뉴 ) 안에 etText를 찾아줌

           if(etText != null){
               etText.setOnEditorActionListener(new TextView.OnEditorActionListener() {
                   @Override
                   public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                    Toast.makeText(getApplicationContext(), v.getText() + ": 입력",Toast.LENGTH_LONG).show(); // 어떤정보를 입력했는지 가져와서 보여줌
                       return false;
