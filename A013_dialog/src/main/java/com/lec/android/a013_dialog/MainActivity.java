...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dig1.setContentView(R.layout.dialog_layout11); // 다이얼로그 화면 등록

        // Dialog 안의 View 객체들 얻어오기
        ivDIgBanner = dig1.findViewById(R.id.ivDlgBanner);
        btnDlgEvent = dig1.findViewById(R.id.btnDlgEvent);

        btnDlgEvent.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ivDIgBanner.setImageResource(R.drawable.face09);
                Toast.makeText(getApplicationContext(),"다이얼로그 버튼을 누름", Toast.LENGTH_LONG).show();
            }
        });

        //현재 Activity 에 Dialog 등록하기
        dig1.setOwnerActivity(MainActivity.this);
        dig1.setCanceledOnTouchOutside(true); // 다이얼로그 바깥영역 클릭시(혹은 back 버튼 클릭시) Hide() 상태가 됨
            // 종료할 것인지 여부 true : 종료됨 , false: 종료안됨

        //#2
        dig2 = new Dialog(this);
        dig2.setContentView(R.layout.dialog_layout12);
        dig2.setCanceledOnTouchOutside(false);
